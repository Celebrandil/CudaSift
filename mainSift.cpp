...
//   Modified for Ada Lovelace (RTX 4060 Ti) by AdaLovelace branch  //
//********************************************************//
...
int main(int argc, char **argv)
{
  ...
  if (limg.empty() || rimg.empty()) {
    std::cerr << "Error: Could not load images from data/ directory" << std::endl;
    return -1;
  }
  ...
  // Benchmark: run extraction 1000 times
